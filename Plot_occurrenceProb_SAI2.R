...
calculate_average_SAI <- function(genus_name){
  ...
  ### Load data
  # Occurrence probability of LGM
  nam <- load(paste("Y://ensemblePredictionBinary_", genus_name, "5km_15Jan19_ensambleprob.data", sep = ""))
  cur <- get(nam)
  # Occurrence probability of the current
  nam.lgm <- load(paste("Y://ensemblePredictionBinary_", genus_name, "5kmLGM_15Jan19prob.data", sep = ""))
  lgm <- get(nam.lgm)
