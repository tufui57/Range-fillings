...
for(i in 1:length(spname)){
  glm1 <- glm(as.formula(paste(spname[i] ,"~",
                               paste(predictor, collapse = "+", sep = "")
  )
  ),
  data = scores.ep, family = binomial)
  fullmodel.AIC <- AIC(glm1)
  
  delta.AIC.sp <- list()
  for(j in 1:length(predictor)){
    glm.i <- glm(as.formula(paste(spname[i] ,"~",
                                  paste(predictor[-j], collapse = "+", sep = "")
    )
    ),
    data = scores.ep, family = binomial)
    
    delta.AIC.sp[j] <- (fullmodel.AIC - AIC(glm.i))
  }
...
save(delta.AIC, file = paste("Y://", genus_name, "deltaAIC_glm.data", sep=""))

#################################################################################################################
### Delta AIC of varibales as importance of variables
#################################################################################################################
load(paste("Y://", genus_name, "deltaAIC_glm.data", sep=""))
...
for(i in 1:length(delta.AIC)){
  # Rank the importance
  imp.rank[[i]] <- rank(-delta.AIC[[i]][,2], ties.method = "min")
}
